...
import static cn.edu.hitsz.compiler.parser.table.Action.ActionKind.Reduce;
...
public class SyntaxAnalyzer {
    ...
    private Stack<Status> statuses = new Stack<>();
    ...
    public void run() {
        ...
        Action action;
        do{
            action = statuses.peek().getAction(tokenList.get(0));
            switch (action.getKind()) {
                case Shift -> {
                    final var shiftTo = action.getStatus();
                    callWhenInShift(statuses.peek(),tokenList.get(0));
                    statuses.push(shiftTo);
                    symbols.push(new Symbol(tokenList.get(0)));
                    tokenList.remove(0);
                }
                case Reduce -> {
                    final var production = action.getProduction();
                    callWhenInReduce(statuses.peek(),production);
                    for(int i=0;i<production.body().size();i++){
                        symbols.pop();
                        statuses.pop();
                    }
                    symbols.add(new Symbol(production.head()));
                    Status status = table.getGoto(statuses.peek(),symbols.peek().getNonTerminal());
                    statuses.push(status);
                }
                case Accept -> {
                    callWhenInAccept(statuses.peek());
                }
                case Error -> {
                    throw new NotImplementedException();
                }
                default -> throw new IllegalStateException("Unexpected value: " + action.getKind());
            }
        }while(action.getKind()!=Accept && action.getKind() != Action.ActionKind.Error);
    }
}
